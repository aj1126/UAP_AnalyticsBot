...
6.  **Verify Coverage:** Run coverage reports using the project's chosen tools.
    For this Node.js project, run `npm test`. Target: >80% coverage for new code.
...
```bash
# Run documentation verification and validation checks
npm run docs:check
```
...
-   Test webhook parsing and metrics generation.
-   Verify worker threads teardown/isolation.
...
5.  **Performance**
    ...
    -   Filesystem caching implemented where needed
...
```bash
git commit -m "feat(telemetry): Add SQLite database connection utility"
git commit -m "fix(worker): Defer thread termination to avoid segfaults"
git commit -m "test(ingestion): Add regression test for corrupted files"
```
...
2.  Restore database from backup or regenerate from ingestion source
3.  Verify data integrity
4.  Document incident

### Security Breach

1.  Rotate all secrets/tokens immediately
...
-   [ ] SQLite database migrations/structures ready
-   [ ] Backup created
...
3.  Deploy release package
4.  Verify database schemas
5.  Verify telemetry monitoring
