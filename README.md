...
Because this engine runs as standard Node.js JavaScript (including `worker_threads` for parallelism), there are no complex C++ build tools or `node-gyp` configurations required on Windows. Simply run:
```bash
npm install
